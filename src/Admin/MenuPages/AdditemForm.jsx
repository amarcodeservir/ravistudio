...
const AdditemForm = ({ closeForm, itemToEdit, fetchItems }) => {
  ...
  return (
    <div
      ref={formRef}
      className="relative w-full max-w-5xl p-8 mx-auto overflow-auto bg-white rounded-lg shadow-lg"
    >
      <button
        onClick={() => closeForm()}
        className="absolute text-2xl text-gray-600 top-4 right-4 hover:text-gray-900"
      >
        <AiOutlineClose />
      </button>
        ...
        <div className="flex justify-center mb-6">
          ...
          >
            {isUploading ? (
              ...
              <>
                <AiOutlineUpload />{" "}
                <span>{itemToEdit ? "Update Product" : "Add Product"}</span>
              </>
            )}
          </button>
        </div>
